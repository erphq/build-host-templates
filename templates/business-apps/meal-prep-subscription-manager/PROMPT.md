PRODUCT: Meal Prep Subscription Manager
WHO AND OUTCOME
A small-business owner needs to complete this job: Tomorrow’s meals, counted correctly. Start with business name and the first real entry; no enterprise setup.

SCREENS AND FIELDS
Main workspace: Tomorrow’s kitchen, with search, relevant date/status filters, a primary New entry action, and a visible list of current work. Selecting an entry opens a detail panel containing editable fields, chronological activity, totals, attachments where specified and the next valid action. Fields: Subscriber, meal plan, quantity, address/route, delivery days, start/end, pauses, cutoff time, renewals. Use an explicit empty state and an optional Load sample business action; sample data never appears to be real.

STATE TRANSITIONS
Active → Paused → Resumed / Ended; day → Planned → Prepared → Delivered. Transitions must be recorded with timestamp. Explain missing requirements inline; do not enable a completion state before its requirements are met. Preserve important previous versions and corrective actions.

BUSINESS RULES
Daily quantity includes active scheduled deliveries excluding pauses accepted before cutoff; define calendar-day versus meal-credit subscription policy. Use decimal monetary arithmetic and USD currency. Validate dates, positive quantities, required identities and duplicate references. Show derived amounts immediately and prevent double submission using server-side idempotency.

ACCEPTANCE EXAMPLE AND OUTPUT
10 subscribers with2 paused =>8 meals if one meal each. Late pause displays cutoff impact. Export route-wise delivery and consolidated kitchen counts.
...
DESIGN IDENTITY: daily kitchen — Kitchen production board.
COLOR TOKENS: background #f8f4e8; primary text #324a29; primary accent #457443. Derive quiet borders and secondary surfaces from this palette. Use contrast-checked status colors and text labels. Do not apply the gallery's black shell to the app.
TYPOGRAPHY: Geist with Georgia menu title. Use self-hosted fonts when available, 16px body, 14px labels, tabular numerals for money. Display sizes below are desktop targets, not mobile minimums.
DESKTOP COMPOSITION: Use a cream kitchen worksheet with a forest-green meal count panel. Oversized 80px total meals is paired with regular/vegetarian or actual dietary counts; on the right show tomorrow menu and substitutions. Bottom strip includes paused subscriptions, new subscriptions and Print kitchen sheet. Include packing counts by route and meal type below. Use 16px panels and plain tabular quantities.
RESPONSIVE BEHAVIOR: Meal counts and cutoff date first, then menu, subscription exceptions and a print/download action.
...
Use the meal prep photograph to fill the right menu panel, showing the actual food prominently. Apply a lower forest overlay behind menu labels; preserve the large green meal-count panel on the left.
Reference photo: https://business-templates.build.host/app-photos/us-meal.jpg
