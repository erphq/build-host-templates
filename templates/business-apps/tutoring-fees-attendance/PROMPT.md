PRODUCT: Tutoring Fees & Attendance
WHO AND OUTCOME
A small-business owner needs to complete this job: Teach without chasing spreadsheets. Start with business name and the first real entry; no enterprise setup.

SCREENS AND FIELDS
Main workspace: September fees, with search, relevant date/status filters, a primary New entry action, and a visible list of current work. Selecting an entry opens a detail panel containing editable fields, chronological activity, totals, attachments where specified and the next valid action. Fields: Student/guardian contact, class group, fee schedule, installments, payment receipts, class session, attendance. Use an explicit empty state and an optional Load sample business action; sample data never appears to be real.

STATE TRANSITIONS
Enrolled → Active / Paused / Left; installment → Due → Part paid → Paid. Transitions must be recorded with timestamp. Explain missing requirements inline; do not enable a completion state before its requirements are met. Preserve important previous versions and corrective actions.

BUSINESS RULES
Attendance is per class session, not one global daily flag; discounts explicitly recorded; fee receipts require actual payment record. Use decimal monetary arithmetic and USD currency. Validate dates, positive quantities, required identities and duplicate references. Show derived amounts immediately and prevent double submission using server-side idempotency.

ACCEPTANCE EXAMPLE AND OUTPUT
$3,000 installment with$1,000 payment leaves$2,000. One student in two classes has separate session attendance. Export class roster, attendance and dues.
...
DESIGN IDENTITY: brightclass — Friendly learning roster.
COLOR TOKENS: background #fcfcf7; primary text #293d57; primary accent #4273cf. Derive quiet borders and secondary surfaces from this palette. Use contrast-checked status colors and text labels. Do not apply the gallery's black shell to the app.
...
DESKTOP COMPOSITION: Use a bright white canvas, blue primary action and a small warm-yellow class badge. The primary view is Attendance with Fees and Classes tabs. Student cards show initials, name and text status; clicking cycles only through valid present/absent/half-day states with an undo affordance. Present/total count stays above the roster and a fee-collection strip below. Use 12px roster corners, 40px heading and ample hit areas.
RESPONSIVE BEHAVIOR: Use a two-column student roster or one-column accessible list, with batch/date controls pinned above.
...
Use the classroom photograph in a rounded upper-right frame next to the title. Overlay only a small yellow class badge; keep the attendance roster on a solid white surface below.
Reference photo: https://images.unsplash.com/photo-1509062522246-3755977927d7?auto=format&fit=crop&w=1200&q=85
Source: https://unsplash.com/s/photos/classroom
