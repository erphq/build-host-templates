PRODUCT: Sales Tax Billing
WHO AND OUTCOME
A small-business owner needs to complete this job: Invoice to payment, in one place. Start with business name and the first real entry; no enterprise setup.

SCREENS AND FIELDS
Main workspace: Invoices, with search, relevant date/status filters, a primary New entry action, and a visible list of current work. Selecting an entry opens a detail panel containing editable fields, chronological activity, totals, attachments where specified and the next valid action. Fields: Business profile, customer billing details and sales-tax settings, invoice lines, user-selected tax treatment, credit notes, receipts. Use an explicit empty state and an optional Load sample business action; sample data never appears to be real.

STATE TRANSITIONS
Draft → Issued → Part paid → Paid; credit notes remain linked to the original. Transitions must be recorded with timestamp. Explain missing requirements inline; do not enable a completion state before its requirements are met. Preserve important previous versions and corrective actions.

BUSINESS RULES
Track invoice numbers uniquely; compute decimal line tax and allocate payments without double counting; credit notes reduce receivables without rewriting issued invoices. Use decimal monetary arithmetic and USD currency. Validate dates, positive quantities, required identities and duplicate references. Show derived amounts immediately and prevent double submission using server-side idempotency.

ACCEPTANCE EXAMPLE AND OUTPUT
Create a $1,080 invoice ($1,000 subtotal plus an illustrative 8% sales tax), receive $500, then issue an $80 credit memo: outstanding $500. Require user-configured tax rates and taxability; 8% is sample data, not a nationwide rate. Export monthly sales and credit notes separately.
...
DESIGN IDENTITY: Billing — Cobalt finance.
COLOR TOKENS: background #f8faff; primary text #19253d; primary accent #345be8. Derive quiet borders and secondary surfaces from this palette. Use contrast-checked status colors and text labels. Do not apply the gallery's black shell to the app.
...
DESKTOP COMPOSITION: A narrow 64px icon rail, compact breadcrumb bar, then a two-column 48/52 workspace. Left: outstanding total with dollar amount, muted cents, seven-day cashflow bars and a dated axis. Right: a white invoice paper with invoice number, customer, line items, separate configurable sales tax, total and Send invoice. Add the searchable invoice list below. Use 12px paper corners and 20px space between major areas.
RESPONSIVE BEHAVIOR: On mobile show invoice list first; opening one uses a full-width invoice detail with a sticky send/export action.
...
Use the desk photograph as a full-height crop behind the LEFT cashflow panel, with a navy overlay, white values and pale-blue bars. Keep the invoice sheet crisp white on the right.
Reference photo: https://images.unsplash.com/photo-1762427355235-dd22e5cb010c?auto=format&fit=crop&w=1100&q=85
Source: https://unsplash.com/photos/desk-with-calculator-binders-notebook-and-glasses--6LEDthF1AI
