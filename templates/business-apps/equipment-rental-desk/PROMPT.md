PRODUCT: Equipment Rental Desk
WHO AND OUTCOME
A small-business owner needs to complete this job: Availability through return, sorted. Start with business name and the first real entry; no enterprise setup.

SCREENS AND FIELDS
Main workspace: Rental reservations, with search, relevant date/status filters, a primary New entry action, and a visible list of current work. Selecting an entry opens a detail panel containing editable fields, chronological activity, totals, attachments where specified and the next valid action. Fields: Equipment units, date/time interval, customer, rate basis, deposit, condition-out/in photos, return time, damage charges. Use an explicit empty state and an optional Load sample business action; sample data never appears to be real.

STATE TRANSITIONS
Reserved → Collected → In use → Returned → Deposit settled. Transitions must be recorded with timestamp. Explain missing requirements inline; do not enable a completion state before its requirements are met. Preserve important previous versions and corrective actions.

BUSINESS RULES
Server-side interval overlap prevention per physical unit; clearly chosen daily/hourly charge rule; deposits are liabilities, not rental revenue. Use decimal monetary arithmetic and USD currency. Validate dates, positive quantities, required identities and duplicate references. Show derived amounts immediately and prevent double submission using server-side idempotency.

ACCEPTANCE EXAMPLE AND OUTPUT
Two simultaneous overlapping reservations for one unit: only one succeeds. Rental2000,deposit3000,damage500=>deposit refund2500 if rental already paid.
...
DESIGN IDENTITY: GEAR / DESK — Night equipment timeline.
COLOR TOKENS: background #171c25; primary text #ebf0f7; primary accent #7e9ffa. Derive quiet borders and secondary surfaces from this palette. Use contrast-checked status colors and text labels. Do not apply the gallery's black shell to the app.
TYPOGRAPHY: Geist sans with Geist Mono timeline labels. Use self-hosted fonts when available, 16px body, 14px labels, tabular numerals for money. Display sizes below are desktop targets, not mobile minimums.
DESKTOP COMPOSITION: Create a dark technical reservation desk. A seven-day availability grid is the focal element: sticky asset-name column, dated day headers and booking bars spanning exact rental intervals. Distinguish available, reserved, on hire, overdue and maintenance with labels, not color alone. Below show available/on-hire counts and New reservation. Booking detail holds deposit, checkout condition, accessories and return inspection.
RESPONSIVE BEHAVIOR: Keep asset names sticky and allow the timeline to scroll; provide an agenda alternative for narrow screens.
...
Use the camera photo as a large dark full-workspace backdrop. Keep the camera visible at the upper right, the title at left, and a readable translucent navy availability timeline across the lower half.
Reference photo: https://images.unsplash.com/photo-1516035069371-29a1b244cc32?auto=format&fit=crop&w=1200&q=85
Source: https://unsplash.com/s/photos/camera-equipment
