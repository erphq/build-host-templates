PRODUCT: Daily Shop Accounts
WHO AND OUTCOME
A small-business owner needs to complete this job: Close the day knowing every dollar. Start with business name and the first real entry; no enterprise setup.

SCREENS AND FIELDS
Main workspace: Today’s register, with search, relevant date/status filters, a primary New entry action, and a visible list of current work. Selecting an entry opens a detail panel containing editable fields, chronological activity, totals, attachments where specified and the next valid action. Fields: Opening cash, cash/ACH/card sales, expenses with payment mode, cash withdrawals, counted closing cash. Use an explicit empty state and an optional Load sample business action; sample data never appears to be real.

STATE TRANSITIONS
Open day → Record movements → Count cash → Close → Reopen with reason. Transitions must be recorded with timestamp. Explain missing requirements inline; do not enable a completion state before its requirements are met. Preserve important previous versions and corrective actions.

BUSINESS RULES
Expected cash=opening+cash sales-cash expenses-withdrawals; ACH/card never increase cash drawer; preserve corrections as adjustments. Use decimal monetary arithmetic and USD currency. Validate dates, positive quantities, required identities and duplicate references. Show derived amounts immediately and prevent double submission using server-side idempotency.

ACCEPTANCE EXAMPLE AND OUTPUT
Opening1000,cash sales500,ACH800,cash expense100 =>expected cash1400. Count1380 =>short20. Export day-close report.
...
DESIGN IDENTITY: daybook — Emerald cash counter.
COLOR TOKENS: background #084c3a; primary text #f6ffe9; primary accent #d0f57e. Derive quiet borders and secondary surfaces from this palette. Use contrast-checked status colors and text labels. Do not apply the gallery's black shell to the app.
...
DESKTOP COMPOSITION: Use an emerald full workspace, generous 48px heading, pale-lime cash balance at 64px, and minimal chrome. Place Money in and Money out as two unboxed figures with icons. Recent entries run in a compact strip below. Primary actions are Receive money, Pay expense and Close day. Opening balance, cash sales, ACH sales and cash expenses remain separately labeled; do not merge bank and cash.
RESPONSIVE BEHAVIOR: On mobile the cash total leads, followed by two large entry actions and a chronological transaction list.
...
Use the retail photograph edge-to-edge behind the workspace. Apply a forest-green gradient strongest on the left and bottom, letting the shop remain visible on the right. Cash totals and entry strips float above it.
