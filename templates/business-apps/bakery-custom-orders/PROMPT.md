PRODUCT: Bakery Custom Orders
WHO AND OUTCOME
A small-business owner needs to complete this job: Every cake, exactly as promised. Start with business name and the first real entry; no enterprise setup.

SCREENS AND FIELDS
Main workspace: Cake orders, with search, relevant date/status filters, a primary New entry action, and a visible list of current work. Selecting an entry opens a detail panel containing editable fields, chronological activity, totals, attachments where specified and the next valid action. Fields: Customer, event date/time, pickup/delivery, cake size/flavour, dietary requests, inscription, reference images, price, deposit. Use an explicit empty state and an optional Load sample business action; sample data never appears to be real.

STATE TRANSITIONS
Enquiry → Confirmed → Baking → Decorating → Ready → Delivered. Transitions must be recorded with timestamp. Explain missing requirements inline; do not enable a completion state before its requirements are met. Preserve important previous versions and corrective actions.

BUSINESS RULES
Confirmation captures specifications; flag changes after production begins; balance=agreed price-payments; preserve uploaded reference. Use decimal monetary arithmetic and USD currency. Validate dates, positive quantities, required identities and duplicate references. Show derived amounts immediately and prevent double submission using server-side idempotency.

ACCEPTANCE EXAMPLE AND OUTPUT
Price3200,deposit1000=>2200 due. Print kitchen job sheet excluding internal customer finance notes and daily delivery list.
...
DESIGN IDENTITY: butter & crumb — Bakery order editorial.
COLOR TOKENS: background #faeff0; primary text #643442; primary accent #924760. Derive quiet borders and secondary surfaces from this palette. Use contrast-checked status colors and text labels. Do not apply the gallery's black shell to the app.
...
DESKTOP COMPOSITION: Lead with an actual reference/design photo for the selected cake in a wide 16:7 crop, with order number and a restrained serif overlay. Below show customer/event, flavor, weight, egg preference, decoration state, pickup and balance. Maintain separate approval, bake/decorate and handover states. Place reference photos, inscription and allergy notes in detail. Use soft 16px corners and fine rose dividers.
RESPONSIVE BEHAVIOR: Photo above details, with pickup and balance always visible near the primary update action.
...
Use the birthday cake photo as the dominant wide editorial image, with a serif lower-left overlay. Customer, flavor, weight, preparation state, pickup and balance stay on the blush surface beneath.
Reference photo: https://images.unsplash.com/photo-1606983340126-99ab4feaa64a?auto=format&fit=crop&w=1200&q=85
Source: https://unsplash.com/s/photos/birthday-cake
