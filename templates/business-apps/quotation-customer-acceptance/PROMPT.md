PRODUCT: Quotation & Customer Acceptance
WHO AND OUTCOME
A small-business owner needs to complete this job: Agree on scope before work begins. Start with business name and the first real entry; no enterprise setup.

SCREENS AND FIELDS
Main workspace: Customer proposal, with search, relevant date/status filters, a primary New entry action, and a visible list of current work. Selecting an entry opens a detail panel containing editable fields, chronological activity, totals, attachments where specified and the next valid action. Fields: Customer, quote version, line items, optional extras, validity, terms, customer name and acceptance timestamp. Use an explicit empty state and an optional Load sample business action; sample data never appears to be real.

STATE TRANSITIONS
Draft → Sent → Accepted / Declined / Expired → Revised version. Transitions must be recorded with timestamp. Explain missing requirements inline; do not enable a completion state before its requirements are met. Preserve important previous versions and corrective actions.

BUSINESS RULES
An accepted version is immutable; optional selections are captured with acceptance; expired quotes cannot be accepted; changes require a new version. Use decimal monetary arithmetic and USD currency. Validate dates, positive quantities, required identities and duplicate references. Show derived amounts immediately and prevent double submission using server-side idempotency.

ACCEPTANCE EXAMPLE AND OUTPUT
Accept version1 then revise price: version1 evidence remains unchanged and version2 is not auto-accepted. Export accepted scope PDF and recorded acceptance details.
...
DESIGN IDENTITY: SCOPE — Editorial proposal.
COLOR TOKENS: background #f8f6ef; primary text #33322e; primary accent #393f91. Derive quiet borders and secondary surfaces from this palette. Use contrast-checked status colors and text labels. Do not apply the gallery's black shell to the app.
...
DESKTOP COMPOSITION: Desktop split 50/50: deep indigo scope navigation on the left and a clean investment sheet on the right. Use 48px serif project title, 54px investment amount, fine milestone rules, numbered sections Overview/Deliverables/Investment, and a quiet acceptance panel. Signature must represent actual acceptance evidence only; the sample can use a clearly marked sample signature. Keep quote revision, expiry and scope change history visible.
RESPONSIVE BEHAVIOR: Use compact section navigation above the proposal, with a sticky Accept proposal action and visible amount.
...
Fill the left proposal panel with interior photography. Use an indigo lower overlay for the serif project title and section navigation; retain the ivory investment sheet on the right.
Reference photo: https://images.unsplash.com/photo-1740999541145-8f84faeb1cbc?auto=format&fit=crop&w=1100&q=85
Source: https://unsplash.com/photos/a-room-with-a-couch-and-a-table-in-it-3hvE2cHRmvk
