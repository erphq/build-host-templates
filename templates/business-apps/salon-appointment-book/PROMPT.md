PRODUCT: Salon Appointment Book
WHO AND OUTCOME
A small-business owner needs to complete this job: A smoother day in the salon. Start with business name and the first real entry; no enterprise setup.

SCREENS AND FIELDS
Main workspace: Today’s appointments, with search, relevant date/status filters, a primary New entry action, and a visible list of current work. Selecting an entry opens a detail panel containing editable fields, chronological activity, totals, attachments where specified and the next valid action. Fields: Customer/contact, service, stylist, duration, price, date/time, walk-in, visit note, payment. Use an explicit empty state and an optional Load sample business action; sample data never appears to be real.

STATE TRANSITIONS
Booked → Arrived → In service → Completed / No-show / Cancelled. Transitions must be recorded with timestamp. Explain missing requirements inline; do not enable a completion state before its requirements are met. Preserve important previous versions and corrective actions.

BUSINESS RULES
Prevent overlaps for one stylist; allow separate stylists concurrently; walk-ins use actual availability; cancellations release slots. Use decimal monetary arithmetic and USD currency. Validate dates, positive quantities, required identities and duplicate references. Show derived amounts immediately and prevent double submission using server-side idempotency.

ACCEPTANCE EXAMPLE AND OUTPUT
A45-minute booking at11:00 blocks11:30 for same stylist but allows11:45. Daily view and customer visit history; no invented SMS confirmations.
...
DESIGN IDENTITY: ATELIER — Salon editorial.
COLOR TOKENS: background #f9f3f1; primary text #493e3c; primary accent #785450. Derive quiet borders and secondary surfaces from this palette. Use contrast-checked status colors and text labels. Do not apply the gallery's black shell to the app.
...
DESKTOP COMPOSITION: Use a 45/55 photo-and-appointment split. Left shows the salon own interior photo, darkened only enough for a short serif title. Right uses warm blush, a slim date strip, stylist selector and stacked bookings with start time, client, service, duration and arrival status. Booking blocks have a slim colored edge; the rest of the interface stays quiet. Use actual salon imagery supplied by the owner in the built app.
RESPONSIVE BEHAVIOR: Reduce photography to a short banner, then make the daily appointment schedule and Book appointment action primary.
...
Use the salon photo in the full-height left 45% of the screen, with a soft dark bottom gradient behind the serif title. The daily appointment column stays light and operational.
Reference photo: https://images.unsplash.com/photo-1600948836101-f9ffda59d250?auto=format&fit=crop&w=1200&q=85
Source: https://unsplash.com/s/photos/salon-interior
