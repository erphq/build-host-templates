PRODUCT: Returns & Replacement Desk
WHO AND OUTCOME
A small-business owner needs to complete this job: Resolve the return, end to end. Start with business name and the first real entry; no enterprise setup.

SCREENS AND FIELDS
Main workspace: Returns desk, with search, relevant date/status filters, a primary New entry action, and a visible list of current work. Selecting an entry opens a detail panel containing editable fields, chronological activity, totals, attachments where specified and the next valid action. Fields: Original order/customer,item/quantity,reason,photos,received date,inspection,resolution,refund reference,replacement dispatch. Use an explicit empty state and an optional Load sample business action; sample data never appears to be real.

STATE TRANSITIONS
Requested → Received → Inspected → Approved / Rejected → Refund pending / Replacement pending → Closed. Transitions must be recorded with timestamp. Explain missing requirements inline; do not enable a completion state before its requirements are met. Preserve important previous versions and corrective actions.

BUSINESS RULES
Returned quantity cannot exceed eligible original quantity; refund confirmed only after a recorded real transaction/reference; reopening keeps history. Use decimal monetary arithmetic and USD currency. Validate dates, positive quantities, required identities and duplicate references. Show derived amounts immediately and prevent double submission using server-side idempotency.

ACCEPTANCE EXAMPLE AND OUTPUT
Return1 of3 units leaves2 unreturned. Replacement shipment alone does not mean delivery. Export pending resolution list and customer summary.
...
DESIGN IDENTITY: loop / returns — Customer recovery desk.
COLOR TOKENS: background #f2f0f8; primary text #433452; primary accent #7956a1. Derive quiet borders and secondary surfaces from this palette. Use contrast-checked status colors and text labels. Do not apply the gallery's black shell to the app.
...
DESKTOP COMPOSITION: Use a light lavender canvas, 56px title and one white case card with 20px corners. Show item/order/customer, current return state, inspection reason and evidence. Two clearly separated resolution options sit beneath: replace with available stock or refund to the recorded payment method. Resolution choice opens required fields; it must not claim a refund occurred without confirmation.
RESPONSIVE BEHAVIOR: Case details stack above two full-width resolution choices, followed by confirmation and history.
...
Use the parcel photograph as a substantial thumbnail in the selected return case header, not a generic package icon. Keep reason, inspection and replacement/refund decisions on clean white panels.
Reference photo: https://images.unsplash.com/photo-1513672494107-cd9d848a383e?auto=format&fit=crop&w=1000&q=85
Source: https://unsplash.com/photos/fyaTq-fIlro
